...
function adjustmentLabel(item: BatchResolveItem): string {
  ...
  if (item.adjustmentType === 'custom_price')
    return `Custom $${item.adjustmentValue?.toFixed(2)} (automatic)`;
  if (item.adjustmentType === 'percentage')
    return `${sign}${item.adjustmentValue}% (automatic)`;
        ...
        <div className="rounded-lg border bg-card overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/50">
              <tr>
                <th className="px-3 py-2.5 text-left font-medium text-sm">
                  Product
                </th>
                <th className="px-3 py-2.5 text-left font-medium text-sm">
                  SKU
                ...
                <th className="px-3 py-2.5 text-right font-medium text-sm">
                  Base Price
                </th>
                <th className="px-3 py-2.5 text-right font-medium text-sm">
                  Adjustment
                  ...
                  <td className="px-3 py-2.5">
                    {item.sourceProfileName ? (
                      <div className="space-y-0.5">
                        ...
                        {item.matchReasonCustomer &&
                          item.matchReasonProduct && (
                            <p className="text-xs text-muted-foreground">
                              {item.matchReasonCustomer} ·{' '}
                              {item.matchReasonProduct} · score{' '}
                              {item.matchScore}
                            </p>
                          )}
                      </div>
