...
export function PricingPage() {
  ...
  const [customerScope, setCustomerScope] =
    useState<CustomerScopeType>('individual');
  ...
  const [productScope, setProductScope] =
    useState<ProductScopeType>('multiple');
  ...
  const {
    data: customerGroups = [],
    isFetching: isFetchingCustomersGroups,
  ...
  const [adjustmentDirection, setAdjustmentDirection] =
    useState<Adjustment['direction']>('increase');
  ...
  const segments = useMemo(
    () => [...new Set(products.map((p) => p.segment))].sort(),
    [products],
  );
  ...
  const selectedProducts = useMemo(() => {
    ...
    if (productScope === 'subCategory')
      return filterSubCategory
        ? products.filter((p) => p.subCategory === filterSubCategory)
        : [];
    if (productScope === 'segment')
      return filterSegment
        ? products.filter((p) => p.segment === filterSegment)
        : [];
    ...
    try {
      ...
      const productPayload = (() => {
        switch (productScope) {
          ...
          case 'multiple':
            return {
              productScope: 'explicit' as const,
              productIds: [...selectedIds],
            };
          case 'all':
          ...
          case 'subCategory':
            return {
              productScope: 'subCategory' as const,
              productFilter: { subCategory: filterSubCategory },
              productIds: [],
            };
          case 'segment':
            return {
              productScope: 'segment' as const,
              productFilter: { segment: filterSegment },
              productIds: [],
            };
        }
      })();
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex overflow-hidden rounded-md border">
                  {(
                    Object.keys(CUSTOMER_SCOPE_LABELS) as CustomerScopeType[]
                  ).map((s) => (
                    <button
                      key={s}
                      type="button"
          ...
          <div className="px-4 pb-4 pt-3 border-t space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex overflow-hidden rounded-md border w-fit">
                {(Object.keys(PRODUCT_SCOPE_LABELS) as ProductScopeType[]).map(
                  (s) => (
                    <button
                      key={s}
                      type="button"
            ...
            {productScope === 'subCategory' ? (
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={filterSegment || undefined}
                  onValueChange={(v) => setFilterSegment(v ?? '')}
                >
                  <SelectTrigger className="w-64">
              ...
              loading ? (
                ...
                <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">
                  Adjustment will apply to all {products.length} products in the
                  catalog.
                </div>
              )
            ) : (
