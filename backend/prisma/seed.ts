...
async function main() {
  ...
  const [pinot, brut, riesling, tussock, lacourte] = await Promise.all([
    prisma.product.create({ data: { title: "High Garden Pinot Noir 2021", sku: "HGVPIN216", subCategory: "Wine", segment: "Red", brand: "High Garden", basePrice: 279.06 } }),
    prisma.product.create({ data: { title: "Koyama Methode Brut Nature NV", sku: "KOYBRUNV6", subCategory: "Wine", segment: "Sparkling", brand: "Koyama Wines", basePrice: 120.0 } }),
    prisma.product.create({ data: { title: "Koyama Riesling 2018", sku: "KOYNR1837", subCategory: "Wine", segment: "Port/Dessert", brand: "Koyama Wines", basePrice: 215.04 } }),
    prisma.product.create({ data: { title: "Koyama Tussock Riesling 2019", sku: "KOYRIE19", subCategory: "Wine", segment: "White", brand: "Koyama Wines", basePrice: 215.04 } }),
    prisma.product.create({ data: { title: "Lacourte-Godbillon Brut Cru NV", sku: "LACBNATNV6", subCategory: "Wine", segment: "Sparkling", brand: "Lacourte-Godbillon", basePrice: 409.32 } }),
  ])
    ...
    data: {
      ...
      items: {
        create: allProducts.map((p) => ({
          ...
          adjustedPrice: computeAdjustedPrice(p.basePrice.toNumber(), "percentage", "decrease", 10),
        })),
      },
    },
  })
    ...
    data: {
      ...
      items: {
        create: sparklingProducts.map((p) => ({
          productId: p.id,
          basePrice: p.basePrice.toNumber(),
          adjustedPrice: computeAdjustedPrice(p.basePrice.toNumber(), "fixed", "decrease", 15),
        })),
      },
    },
  })
        ...
        create: [
          {
            ...
            adjustedPrice: computeAdjustedPrice(pinot.basePrice.toNumber(), "custom_price", "decrease", 95),
          },
        ],
      },
    },
  ...
  console.log("Seeded: 1 admin user, 6 customers, 2 groups, 2 memberships, 5 products, 3 profiles")
}
