...
export function mapItem(i: PrismaItem & { product: PrismaProduct }): PricingProfileItem {
  return {
...
export function mapProfile(p: PrismaProfileWithRelations): PricingProfile & {
  ...
  const base = {
    ...
    adjustmentDirection: p.adjustmentDirection as PricingProfile["adjustmentDirection"],
  ...
  const withGroup =
    p.customerGroupId != null
      ...
      : {}
  const withFilter =
    p.productFilter != null ? { productFilter: p.productFilter as ProductFilter } : {}
